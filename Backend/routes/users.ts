...
router.post("/courses/:id", authenticateJwt, async (req: UpdatedRequest, res:Response) => {
  const course = await Course.findById(req.params.id)
  if(course){
    const user = await User.findOne({username: req.user.username})
    if(user){
      user.purchasedCourses.push(course as any);
      await user.save();
...
router.get("/purchasedCourses", authenticateJwt, async (req:UpdatedRequest, res:Response) => {
